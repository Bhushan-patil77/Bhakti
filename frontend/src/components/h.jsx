...
{selectedUser ? (
  <div className="flex flex-col h-full">
        ...
        <div className="ml-3">
          ...
          <p className="text-xs text-gray-400">
            {onlineUsers.includes(selectedUser._id) ? (
              usersTyping.includes(selectedUser._id) ? 'typing...' : 'online'
    ...
    <div
      ref={msgWindow1}
      className="MESSAGES flex-1 overflow-y-auto no-scrollbar px-4 pt-4 bg-gray-900"
      style={{ maxHeight: 'calc(100vh - 200px)' }} // Dynamically calculate the height of messages
    >
      {messages.length > 0 && messages.map((message, index) => {
        const messageTime = new Date(message.createdAt).toLocaleTimeString();
        ...
        if (message.senderId === loggedInUser._id) {
          return (
            <div key={index} className="mt-4 text-right">
              <div className="flex justify-end">
                <div className="bg-gray-800 text-white p-3 rounded-lg max-w-[400px] flex flex-col">
                  {message.images?.length > 0 && (
                    <div className="flex flex-wrap justify-start gap-2 mb-2">
                      {message.images.map((image, i) => (
                        <img key={i} className="w-[175px] h-[175px] object-cover rounded-md" src={image} alt="" />
                      ))}
                    </div>
                  )}
                  <p className="text-sm break-words text-start">{message.text}</p>
                </div>
              </div>
              <span className="text-xs text-gray-400 flex justify-end items-center gap-3 min-h-[30px] pr-2">
                <span>{messageTime}</span>
                {!message.delivered && !message.seen ? <span className="inline-block text-xl text-white checkmark"><BiCheck /></span> : ''}
                {message.delivered && !message.seen ? <span className="inline-block text-xl text-white checkmark"><BiCheckDouble /></span> : ''}
                {message.delivered && message.seen ? <span className="inline-block text-green-600 text-xl checkmark"><BiCheckDouble /></span> : ''}
              </span>
            </div>
          );
        } else {
          return (
            <div key={index} seen={message.seen ? 'true' : 'false'} messageid={message._id} className="mt-4">
              <div className="flex justify-start">
                <div className="bg-gray-800 text-white p-3 rounded-lg max-w-[400px] flex flex-col">
                  {message.images.length > 0 && (
                    <div className="flex flex-wrap justify-start gap-2 mb-2">
    ...
    {loading && numberOfFileSelected > 0 && (
      <div className="IMAGE-PRIVIEW-SKELETON min-h-[90px] flex items-center gap-2 p-2 bg-slate-800 no-scrollbar overflow-auto">
        {Array(numberOfFileSelected).fill(0).map((dummyImage, index) => (
          <span key={index} className="w-[70px] h-[70px] rounded shadow-lg flex justify-center items-center flex-col gap-1">
            <FiLoader className="animate-spin text-white" />
            <p className="text-[9px] text-white">Loading...</p>
          </span>
        ))}
      </div>
    )}
    ...
    {/* Image Preview */}
    {!loading && uploadedUrls.length > 0 && (
      <div className="IMAGE-PRIVIEW min-h-[90px] flex items-center gap-2 p-2 bg-slate-800 no-scrollbar overflow-auto">
        {uploadedUrls.map((imageUrl, index) => (
          <img key={index} src={imageUrl} className="w-[70px] h-[70px] object-cover rounded shadow-lg" alt="" />
        ))}
      </div>
    )}

    {/* Input Section */}
    <div className="INPUT-SECTION border-t border-gray-700 p-4 bg-gray-800">
      <div className="flex">
        <input
          ref={textInputRef}
          type="text"
        ...
        <input
          ...
          multiple
        />
        <label htmlFor="imageInput" className="flex justify-center items-center p-1 bg-gray-700 cursor-pointer">
          <MdAttachment className="w-[30px] h-[20px] text-white" />
        </label>
        <button
          disabled={loading}
          className={`text-white p-2 rounded-r-lg hover:bg-blue-600 ${loading ? 'cursor-not-allowed disabled bg-slate-400' : 'bg-blue-500'}`}
          onClick={() => {
            sendMessageWithUploadedImages(loggedInUser._id, selectedUser._id, message, uploadedUrls);
            setMessage('');
            setPreviewImages([]);
